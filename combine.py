...
def run(file_name):
    ...
    train_labels = np.array(train_labels,dtype=np.uint8) #as emnist
    np.savez(file_name.split('/')[0],train_images,train_labels)
    print('FILES SAVED!!')
    end_time = time.time()
    print('Time used(minutes): ', (end_time-start_time)/60)

def test(file_name):
    x,y = load_data(file_name+'.npz')
    num = 0
    print('x: ',x.shape)
    print('y: ',y.shape)
##    for img in range(123,len(x)):
##        if (num == 10):
##            break
...
def test_each():
    font = 'behind-brown-eyes/Behind Brown Eyes-TTF'
    alphabets = get_alphabets(font)
    img = alphabets[3]
    image_object = Image(img)
...
if __name__ == '__main__':
    file_name = 'elabor/Elabor'
    run(file_name)
    test(file_name.split('/')[0])
    #operations = generate_permutaion_operation()
    #test_each()
