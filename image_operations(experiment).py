...
class Erosion(Image):
    def __init__(self, images):
        ...
        if isinstance(images, list): # in case images passed from other operation
            ...
            self.images = [images] * 4 # because dilation has 4 posibilities
        self.operation = 'e'
...
class Zoom(Image):
    def __init__(self, images):
        ...
        if isinstance(images, list): # in case images passed from other operation
            ...
            self.images = [images] * 4 # because zoom has 4 posibilities (2 IN and 2 OUT)
        self.operation = 'z'
        self.generate_Zoom()
...
class GaussianBlur(Image):
    def __init__(self, images):
        ...
        if isinstance(images, list): # in case images passed from other operation
            ...
            self.images = [images] * 2 # because Gaussian blur has 2 posibilities (3x3 amd 1x3 kernel)
...
if __name__ == '__main__':
    ...
    for permute in operations: # each permutation
        for character in range(97,123): # 'a' to 'z'
            if (not os.path.isdir('./'+destination_folder+'/'+chr(character))):
                os.mkdir('./'+destination_folder+'/'+chr(character))
