...
##    save_images(image_dir, destination_folder,processed_image)
    ...
    for permute in operations: # each permutation
        destination_folder = ''
##        for action in permute: # each operation
##            destination_folder += dictionary_operaion(action)
            
        
        for filename in glob.glob(os.path.join(folder_path, '*.png')):
            image_dir = filename.replace('\\','/')
            rename_image = image_dir.split('/')
            alphabet = rename_image[len(rename_image)-1].split('.')[0]
...
####                do_hnm = HitAndMiss(image_object)
##
...
##        image_object = Image(a)
##
##        processed_image = GaussianBlur(image_object).get_image()
##        save_images(image_dir, destination_folder,processed_image)
